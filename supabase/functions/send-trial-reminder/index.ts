...
function createTrialEndingEmail(userName: string, expiryDate: string, pricingUrl: string) {
  const content = `
    ...
    <p style="margin: 0 0 16px 0; font-size: 16px; line-height: 1.6; color: #333333;">
      Just a heads up — your 14-day free trial ends in 3 days.
    </p>

    <p style="margin: 0 0 24px 0; font-size: 16px; line-height: 1.6; color: #333333;">
      To keep access to your calendars and client pages, upgrade to a paid plan before ${expiryDate}.
    </p>

    <table width="100%" cellpadding="0" cellspacing="0" style="margin: 24px 0;">
      <tr>
        <td align="center">
          <a href="${pricingUrl}" style="display: inline-block; padding: 14px 32px; background-color: ${BRAND_COLOR}; color: #1a1a1a; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 16px;">
            Choose a plan →
          </a>
        </td>
      </tr>
    ...
    <p style="margin: 24px 0 0 0; font-size: 14px; line-height: 1.6; color: #666666;">
      Not ready yet? No worries — your data will be saved for 30 days after your trial ends.
    </p>
  ...
  return {
    subject: 'Your Content Charm trial ends in 3 days',
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!userEmail || !userName || !expiryDate || !pricingUrl) {
      return new Response(
    ...
    if (!resendApiKey || resendApiKey === "re_123456789") {
      console.log("Development mode: Trial reminder would be sent to:", userEmail);
      console.log("Subject: Your Content Charm trial ends in 3 days");
    ...
    const emailTemplate = createTrialEndingEmail(userName, expiryDate, pricingUrl);
